import Rpplh from "../models/RpplhModel.js";
import path from "path";
import fs from "fs";
import {Op} from "sequelize";

export const getRpplh = async(req, res)=>{
    const page = parseInt(req.query.page) || 0;
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search_query || "";
    const offset = limit * page;
    const totalRows = await Rpplh.count({
        where:{
            [Op.or]: [{judul:{
                [Op.like]: '%'+search+'%'
            }}, {tanggalPelaksanaan:{
    ...
    const result = await Rpplh.findAll({
        where:{
            [Op.or]: [{judul:{
                [Op.like]: '%'+search+'%'
            }}, {tanggalPelaksanaan:{
...
export const getRpplhById = async(req, res)=>{
    try {
        const response = await Rpplh.findOne({
            where:{
                id : req.params.id
            }
        });
...
export const saveRpplh = async(req, res)=>{
    ...
    file.mv(`./public/images/${fileName}`, async(err)=>{
        file2.mv(`./public/images/${fileName2}`, async(err)=>{
            ...
            try {
                await Rpplh.create({judul: judul,
                    nomor: nomor,
                    status: status,
...
export const updateRpplh = async(req, res)=>{
    
    const andal = await Rpplh.findOne({
        where:{
            id : req.params.id
        }
    });
    ...
    try {
        await Rpplh.update({judul: judul,
            status: status,
            nomor: nomor,
            tanggalPelaksanaan: tanggalPelaksanaan,
            pemrakarsa: pemrakarsa,
...
export const deleteRpplh = async(req, res)=>{
    const andal = await Rpplh.findOne({
        where:{
            id : req.params.id
        }
