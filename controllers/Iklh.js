import Iklh from "../models/IklhModel.js";
import path from "path";
import fs from "fs";
import {Op} from "sequelize";

export const getIklh = async(req, res)=>{
    const page = parseInt(req.query.page) || 0;
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search_query || "";
    const offset = limit * page;
    const totalRows = await Iklh.count({
        where:{
            [Op.or]: [{judul:{
                [Op.like]: '%'+search+'%'
            }}, {tanggalPelaksanaan:{
    ...
    const result = await Iklh.findAll({
        where:{
            [Op.or]: [{judul:{
                [Op.like]: '%'+search+'%'
            }}, {tanggalPelaksanaan:{
...
export const getIklhById = async(req, res)=>{
    try {
        const response = await Iklh.findOne({
            where:{
                id : req.params.id
            }
        });
...
export const saveIklh = async(req, res)=>{
...
export const updateIklh = async(req, res)=>{
    
    const andal = await Iklh.findOne({
        where:{
            id : req.params.id
        }
    });
...
export const deleteIklh = async(req, res)=>{
    const andal = await Iklh.findOne({
        where:{
            id : req.params.id
        }
