import Sppl from "../models/SpplModel.js";
import path from "path";
import fs from "fs";
import {Op} from "sequelize";

export const getSppl = async(req, res)=>{
    const page = parseInt(req.query.page) || 0;
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search_query || "";
    const offset = limit * page;
    const totalRows = await Sppl.count({
        where:{
            [Op.or]: [{nomor:{
                [Op.like]: '%'+search+'%'
            }},{status:{
    ...
    const result = await Sppl.findAll({
        where:{
            [Op.or]: [{nomor:{
                [Op.like]: '%'+search+'%'
            }},{status:{
...
export const getSpplById = async(req, res)=>{
    try {
        const response = await Sppl.findOne({
            where:{
                id : req.params.id
            }
        });
...
export const saveSppl = async(req, res)=>{
...
export const updateSppl = async(req, res)=>{
    
    const sppl = await Sppl.findOne({
        where:{
            id : req.params.id
        }
    });
    if(!sppl) return res.status(404).json({msg: "No Data Found"});
    
    let fileName = "";
    let fileName2 = "";
    if(req.files === null){
        fileName = sppl.dokumentasi;
        fileName2 = sppl.beritaAcara;
    }else{
        const file = req.files.file;
        const file2 = req.files.file2;
        const fileSize = file.data.length;
        ...
        const filepath = `./public/images/${sppl.dokumentasi}`;
        fs.unlinkSync(filepath);
        const filepath2 = `./public/images/${sppl.beritaAcara}`;
...
export const deleteSppl = async(req, res)=>{
    const sppl = await Sppl.findOne({
        where:{
            id : req.params.id
        }
