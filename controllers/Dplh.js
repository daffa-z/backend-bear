import Dplh from "../models/DplhModel.js";
import path from "path";
import fs from "fs";
import {Op} from "sequelize";

export const getDplh = async(req, res)=>{
    const page = parseInt(req.query.page) || 0;
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search_query || "";
    const offset = limit * page;
    const totalRows = await Dplh.count({
        where:{
            [Op.or]: [{nomor:{
                [Op.like]: '%'+search+'%'
            }},{status:{
    ...
    const result = await Dplh.findAll({
        where:{
            [Op.or]: [{nomor:{
                [Op.like]: '%'+search+'%'
            }},{status:{
...
export const getDplhById = async(req, res)=>{
    try {
        const response = await Dplh.findOne({
            where:{
                id : req.params.id
            }
        });
...
export const saveDplh = async(req, res)=>{
...
export const updateDplh = async(req, res)=>{
    
    const dplh = await Dplh.findOne({
        where:{
            id : req.params.id
        }
    });
    if(!dplh) return res.status(404).json({msg: "No Data Found"});
    
    let fileName = "";
    let fileName2 = "";
    if(req.files === null){
        fileName = dplh.dokumentasi;
        fileName2 = dplh.beritaAcara;
    }else{
        const file = req.files.file;
        const file2 = req.files.file2;
        const fileSize = file.data.length;
        ...
        const filepath = `./public/images/${dplh.dokumentasi}`;
        fs.unlinkSync(filepath);
        const filepath2 = `./public/images/${dplh.beritaAcara}`;
...
export const deleteDplh = async(req, res)=>{
    const dplh = await Dplh.findOne({
        where:{
            id : req.params.id
        }
